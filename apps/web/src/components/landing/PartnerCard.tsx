...
function PartnerCard({
	...
	// Responsive percentage-based logo width so it scales with the planet container
	// const logoWidthClass = is_title
	// 	? "w-[46%] sm:w-[40%] lg:w-[36%]"
	// 	: isWordmark
	// 		? "w-[44%] sm:w-[36%] lg:w-[32%]"
	// 		: "w-[40%] sm:w-[34%] lg:w-[30%]";
	const logoShadowClass = isWordmark
	...
	return (
		...
		>
			...
			>
				{/* Glow Layer */}
				<div
					className={`pointer-events-none absolute left-1/2 top-1/2 z-0 ${is_title ? "sun-glow-pulse" : ""}`}
					style={{
				...
				>
					...
					{/* Proportional Logo overlay (scales with planet) */}
					<div
						className={`absolute left-1/2 top-1/2 z-20 aspect-square w-[50%] -translate-x-1/2 -translate-y-1/2`}
					>
						<Image
							...
							quality={100}
							priority={true}
							className={`object-contain ${logoShadowClass}`}
						/>
					</div>
				</div>

				{/* Logo moved inside planet container for proportional sizing */}
			</div>
