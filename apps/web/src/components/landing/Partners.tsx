...
const CARD_GAP_FACTOR = 0.7; // each card width <= 70% of average arc gap (lower => more breathing room)
...
export default function Partners() {
	...
	const titlePartners = partnerData.partners.filter(
		(p) => p.tier === "Title Sponsor",
	);
	...
	return (
		<section className="relative min-h-[800px] w-full min-w-[375px] overflow-hidden border-4">
			{/* ===== centered title sponsor (the sun) ===== */}
			{titlePartners.length > 0 && (
				<div className="flex w-full flex-col items-center justify-center py-16">
					<h2 className="font-texatbold mb-6 text-center text-4xl font-black tracking-wide text-purple-500 drop-shadow-lg md:text-5xl">
						Title Sponsor
					</h2>
					<div className="flex translate-y-16 items-center justify-center">
						<PartnerCard
							partner={titlePartners[0]}
							is_title={true}
						/>
					</div>
				</div>
			...
			{/* ===== orbit tiers ===== */}
			{tiers.map((tier) => {
				const tierPartners = partnerData.partners.filter(
					(p) => p.tier === tier,
				);
				...
				tierPartners.forEach((p, i) => {
					...
					if (i < tierPartners.length - 1) {
						followers.push({
							type: "rock",
							logo: rockImgs[
								Math.floor(Math.random() * rockImgs.length)
							],
						});
					}
				});
				...
				const cardSize = Math.min(
					desiredCard,
					Math.floor(avgGapPx * CARD_GAP_FACTOR),
				);
				...
				return (
					...
					>
						<h2
							className={`font-texatbold mt-32 text-center text-4xl font-black tracking-wide drop-shadow-lg md:text-5xl ${
								tier === "Gold Sponsor"
									? "text-yellow-600"
									: tier === "Silver Sponsor"
										? "text-gray-400"
											...
											{f.type === "card" && f.data ? (
												...
												<div
													className={[
														"h-full w-full",
														[
															"animate-float-wobble-1",
															"animate-float-wobble-2",
															"animate-float-wobble-3",
														][index % 3],
													].join(" ")}
												>
													<Image
