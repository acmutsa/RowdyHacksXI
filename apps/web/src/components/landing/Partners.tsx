...
export default function Partners() {
	...
	const titlePartner = hasPartners
		? (partnerData.partners[0] as Partner)
		: undefined;
	...
	const mainSponsors = useMemo(
		() =>
			(partnerData.partners || []).filter(
				(p) =>
					p.tier === "Bronze Sponsor" || p.tier === "Rowdy Partner",
			),
		[],
	);
...
function SunHero({ partner }: { partner: Partner }) {
	...
	return (
		...
		>
			...
			<motion.h2
				className="font-texatbold pointer-events-none sticky top-16 z-20 text-center text-4xl font-black tracking-wide text-purple-500 drop-shadow-lg md:text-5xl"
...
function MergedOrbit({
	...
	return (
		...
		>
			...
			<div
				ref={stageRef}
				className="relative -mt-8 w-full overflow-visible"
				style={
					{
